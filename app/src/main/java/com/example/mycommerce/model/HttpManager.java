...
import com.example.mycommerce.model.api.home.HomeTpApi;
import com.example.mycommerce.model.api.kuangown.IKuangOwnTpApi;
import com.example.mycommerce.model.api.sort.SortTpApi;
import com.example.mycommerce.model.api.special.Imall;
import com.example.mycommerce.utils.DigestUtils;
...
public class HttpManager {
    ...
    public HomeTpApi getTpApi(){
        if(tpApi == null){
            tpApi = getRetrofit(Constants.Base_TPUrl).create(HomeTpApi.class);
        }
        return tpApi;
    }

    public SortTpApi getStApi(){
        if(stApi == null){
            stApi = getRetrofit(Constants.Base_TPUrl).create(SortTpApi.class);
        }
        return stApi;
    }

    public Imall getMallApi(){
        if(mallApi == null){
            mallApi = getRetrofit(Constants.Base_Url).create(Imall.class);
        }
        return mallApi;
    }
    public IKuangOwnTpApi geIKuangOwnTpApi(){
        if(iKuangOwnTpApi == null){
            iKuangOwnTpApi = getRetrofit(Constants.Base_Url).create(IKuangOwnTpApi.class);
        }
        return iKuangOwnTpApi;
    }
}
